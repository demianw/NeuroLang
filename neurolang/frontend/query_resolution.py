...
class NeuroSynthMixin:
    ...
    def load_neurosynth_term_study_associations(
        ...
        """
        Load TF-IDF values for each (term, study) association within the
        Neurosynth database.

        This loads a tuple of (id, str, float) for each (term, study)
        association.
    ...
    def load_neurosynth_study_ids(
        ...
        fe.Symbol
            fe.Symbol associated to the resulting set.
        """
        metadata = fetch_study_metadata(data_dir=data_dir, version=version)
        if name is None:
            name = str(uuid1())
        if convert_study_ids:
            return self.add_tuple_set(
    ...
    def load_neurosynth_mni_peaks_reported(
        ...
        """
        Load the coordinates for the peaks reported by studies in the
        Neurosynth dataset. Coordinates for the peaks are in MNI space.

        This loads a tuple of (id, int, int, int) values for each reported
        peak.
        ...
        """
        peak_data = get_ns_mni_peaks_reported(
            data_dir=data_dir,
            version=version,
            convert_study_ids=convert_study_ids,
        )
